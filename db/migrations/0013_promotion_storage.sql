-- Tupstore — Admin Promotion Storage migration
-- Creates the `promotion-images` Storage bucket (public read) and
-- admin-gated policies on storage.objects, restricted to
-- public.is_admin() = true (defined in db/migrations/0001_admin_foundation.sql).
-- Follows the exact pattern established in
-- 0003_admin_category_storage.sql/0006_admin_product_storage.sql/
-- 0009_social_video_storage.sql.
--
...
-- social_videos, social_video_products, promotions, promotion_products, or
-- any of their policies/grants. Does NOT touch the existing category-images,
-- product-images, social-video-thumbnails, or social-videos buckets, or
-- ANY of their policies — every policy below is scoped to
-- bucket_id = 'promotion-images' only, and every policy name below is
-- distinct from every prior bucket's own policy names, so nothing here can
-- collide with or shadow an existing policy.
--
...
-- error. The admin SELECT policy below is included from the start this
-- time, specifically to avoid repeating that same two-step gap a third
-- time on a fourth bucket.
--
-- Object naming (documented here for whenever the future Admin upload code
-- is written — not implemented by this migration, which only creates the
-- bucket/policies): flat `${crypto.randomUUID()}.${extension}`, matching
-- category-images'/social-video-thumbnails' existing flat convention
-- (never product-images' per-record-folder convention) — a promotion's
-- own image is a single card image, not a multi-image gallery, so there is
-- no per-record collection to namespace by folder.
--
-- No automatic deletion of a previous image when a promotion record is
...
-- storefront load promotion images via Storage's public URL scheme without
...
-- data-plane SELECT below are gated by policy.
--
-- `file_size_limit`/`allowed_mime_types` are set directly on the bucket
-- row, matching social-video-thumbnails'/social-videos' precedent (0009) —
-- defense-in-depth even if a future admin upload path's own client-side
-- check were ever bypassed or buggy.
--
-- 5 MB / JPG-PNG-WebP matches the existing category-images/product-images/
-- social-video-thumbnails convention exactly (adminStorageApi.js's/
-- adminProductApi.js's own MAX_IMAGE_BYTES = 5 * 1024 * 1024). JPG and
-- JPEG are the same MIME type (image/jpeg) — no separate entry needed.
...
-- Every policy is scoped to bucket_id = 'promotion-images' so it can never
...
create policy "Admins can upload promotion images" on storage.objects
  ...
  with check (bucket_id = 'promotion-images' and public.is_admin());

create policy "Admins can update promotion images" on storage.objects
  for update to authenticated
  using (bucket_id = 'promotion-images' and public.is_admin())
  with check (bucket_id = 'promotion-images' and public.is_admin());
...
-- Admin SELECT — required for a future admin upload/manage UI's list()/
...
create policy "Admins can view promotion images" on storage.objects
